...
def test_process_data_temporal_split():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        msg = f"Expected 3 training records (year <= 2010), got {len(train_df)}"
        assert len(train_df) == 3, msg
...
def test_process_data_preserves_audio_features():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        for feature in expected_audio_features:
            msg_train = f"Missing audio feature '{feature}' in training data"
            assert feature in train_df.columns, msg_train
            msg_prod = f"Missing audio feature '{feature}' in production data"
            assert feature in prod_df.columns, msg_prod
...
def test_process_data_year_boundary_condition():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        msg_train = f"Expected years {{2009, 2010}} in train, got {set(train_df['year'])}"
        assert set(train_df["year"]) == {2009, 2010}, msg_train
        msg_prod = f"Expected years {{2011, 2012}} in prod, got {set(prod_df['year'])}"
        assert set(prod_df["year"]) == {2011, 2012}, msg_prod
